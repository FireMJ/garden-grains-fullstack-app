...
// Fallback banner gradient with new color scheme
const bannerFallbacks = [
  `linear-gradient(135deg, ${colors.darkAzure} 0%, ${colors.lightAzure} 100%)`,
  `linear-gradient(135deg, ${colors.sageGreen} 0%, #8A9B6E 100%)`,
  `linear-gradient(135deg, ${colors.accent} 0%, #e68e42 100%)`,
...
function VerticalPromoBanner() {
  return (
    <div className="fixed left-4 top-1/2 transform -translate-y-1/2 z-30 hidden lg:block">
      <div className={`bg-[${colors.accent}] text-white p-4 rounded-lg shadow-lg max-w-[200px]`}>
...
const DynamicBanner: React.FC = () => {
  ...
  if (!isClient) {
    return (
      <div className={`h-64 sm:h-80 md:h-96 bg-gradient-to-br from-[${colors.darkAzure}] to-[${colors.lightAzure}] flex items-center justify-center`}>
        <div className="text-center text-white">
          <div className={`animate-spin rounded-full h-12 w-12 border-b-2 border-[${colors.accent}] mx-auto mb-4`}></div>
          <p>Loading...</p>
        </div>
      </div>
    );
  ...
  return (
    ...
    >
      ...
      {/* Banner Overlay with updated text */}
      <div className="absolute inset-0 bg-gradient-to-r from-black/40 to-black/20 flex items-center justify-center">
        <div className="text-center text-white px-4 max-w-4xl">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
            <Link
              href="/menu"
              className={`bg-[${colors.accent}] text-white px-8 py-4 rounded-lg hover:opacity-90 transition shadow-lg text-lg font-semibold inline-block min-w-[180px] text-center`}
            >
              Order Now
            </Link>
            <Link
        ...
        {bannerImages.map((_, index) => (
          <button
            ...
            className={`w-3 h-3 rounded-full transition ${
              index === currentIndex ? `bg-[${colors.accent}]` : 'bg-white/50'
            }`}
    ...
    <section className="relative overflow-hidden py-20">
      ...
      <div 
        ...
        style={{
          ...
          background: `linear-gradient(rgba(0, 91, 150, 0.9), rgba(0, 91, 150, 0.7)), url('/images/healthy-bg.jpg')`,
...
// -------------------- Menu Section with Garden & Grains Colors --------------------
function MenuSection() {
  ...
  return (
    <section className="py-20 px-6" style={{ backgroundColor: colors.sageGreen }}>
            ...
            {categories.map(cat => (
              <button
                ...
                className={`px-5 py-2 rounded-full transition ${
                  activeCategory === cat 
                    ? `bg-[${colors.darkAzure}] text-white` 
                    : `bg-white text-[${colors.darkAzure}] hover:bg-[${colors.lightAzure}] hover:text-white`
                }`}
              >
                {cat}
              </button>
            ...
            >
              <div className="relative h-48">
                <div className={`absolute inset-0 flex items-center justify-center ${
                  ...
                  item.category === 'salad' ? 'bg-gradient-to-br from-[#A3B899] to-[#8A9B6E]' :
                  item.category === 'stir-fry' ? 'bg-gradient-to-br from-[#F4A261] to-[#e68e42]' :
                  'bg-gradient-to-br from-[#6C7B58] to-[#8A9B6E]'
                }`}>
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  {item.tags.map((tag, i) => (
                    <span 
                      ...
                      style={{ 
                        backgroundColor: `${colors.darkAzure}15`, 
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {features.map((feature, index) => (
            <div 
              key={index} 
              className="text-center p-6 rounded-xl transition hover:shadow-xl"
...
function ActionButtonsSection() {
  return (
    <section className="py-16 px-6">
      <div className="max-w-6xl mx-auto">
        <div className="bg-gradient-to-r from-[#005B96] to-[#1C7AB0] rounded-2xl p-8 md:p-12 text-white">
          ...
          <div className="flex flex-col md:flex-row justify-center items-center gap-6">
            ...
            <Link 
              href="/menu" 
              className="flex-1 max-w-xs bg-white text-[#005B96] px-8 py-5 rounded-xl hover:bg-gray-100 text-xl font-bold transition flex flex-col items-center justify-center gap-3 min-h-[140px] shadow-lg hover:shadow-xl"
            >
            ...
            <Link 
              href="/catering" 
              className="flex-1 max-w-xs bg-[#F4A261] text-white px-8 py-5 rounded-xl hover:bg-[#e68e42] text-xl font-bold transition flex flex-col items-center justify-center gap-3 min-h-[140px] shadow-lg hover:shadow-xl"
            >
              <div className="text-4xl">🎉</div>
              <div>
            ...
            <Link 
              href="/reserve" 
              className="flex-1 max-w-xs bg-[#A3B899] text-white px-8 py-5 rounded-xl hover:bg-[#8A9B6E] text-xl font-bold transition flex flex-col items-center justify-center gap-3 min-h-[140px] shadow-lg hover:shadow-xl"
            >
              <div className="text-4xl">🪑</div>
              <div>
...
function ConnectSection() {
  return (
    <section className="py-16 px-6 text-center" style={{ backgroundColor: colors.darkAzure }}>
