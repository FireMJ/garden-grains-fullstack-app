...
export default function ToastieDetailPage() {
  ...
  const toastieItem = toasties.find(item => item.slug === slug);

  // State for customization
  const [selectedAddOns, setSelectedAddOns] = useState<AddOn[]>([]);
  const [selectedFries, setSelectedFries] = useState<FriesOption | null>(null);
  const [selectedJuice, setSelectedJuice] = useState<{ size: string; option: JuiceOption } | null>(null);
  const [selectedDips, setSelectedDips] = useState<FriesOption[]>([]);
    ...
    return (
      <div className="min-h-screen bg-[#1E4259] flex items-center justify-center">
        <div className="text-center text-white">
          <h1 className="text-4xl font-bold mb-4">Toastie Not Found</h1>
          <button 
            onClick={() => router.push("/menu/toasties")}
            className="bg-[#F4A261] text-white px-6 py-3 rounded-lg hover:bg-[#e68e42] transition"
          >
            Back to Toasties Menu
          </button>
        </div>
      </div>
    );
  ...
  // Separate fries from dips
  const friesOptions = toastieItem.friesUpsell.filter(item => item.price > 0);
  ...
  const handleAddToCart = () => {
    const cartItem = {
      ...
      price: toastieItem.price,
      quantity: quantity,
      total: itemTotal,
      addOns: selectedAddOns,
      fries: selectedFries || undefined, // Convert null to undefined to match CartItem interface
      juice: selectedJuice,
      dips: selectedDips,
  ...
  const toggleFries = (fries: FriesOption) => {
    setSelectedFries(prev => 
      prev?.id === fries.id ? null : fries
    );
  };

  const toggleDip = (dip: FriesOption) => {
        ...
        <div className="grid md:grid-cols-2 gap-8">
          ...
          <div className="relative h-80 md:h-96 rounded-2xl overflow-hidden">
            ...
            <div className="absolute inset-0 bg-gradient-to-br from-[#6C7B58] to-[#8A9B6E] flex items-center justify-center">
              <span className="text-white/80 text-lg">Toastie Image</span>
            </div>
          </div>
          ...
          <div className="space-y-6">
            <div>
              ...
              <p className="text-gray-300 mb-4">
                {toastieItem.description}
              </p>
              <div className="text-2xl font-bold text-green-300">
                R{toastieItem.price.toFixed(2)}
              </div>
            </div>
                ...
                <div className="space-y-2">
                  {dipOptions.map((dip) => (
                    <label key={dip.id} className="flex items-center gap-3 cursor-pointer">
                      <input
                        type="checkbox"
                        checked={selectedDips.some(d => d.id === dip.id)}
            ...
            {/* Juice Upsell */}
            {toastieItem.juiceUpsell.length > 0 && (
              <div className="space-y-3">
                <h3 className="text-lg font-semibold text-[#F4A261]">Add a Fresh Juice</h3>
                <div className="space-y-2">
                  {toastieItem.juiceUpsell.map((juiceGroup) => (
