...
import { Truck, Loader, CheckCircle } from "lucide-react";
...
interface OrderItem {
  ...
  fries?: { name: string; price: number; quantity?: number }[];
  juices?: { name: string; size?: string; price: number; quantity?: number }[];
}
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5 hover:shadow-md transition-all">
      ...
      <div className="flex justify-between items-center">
        <p className="font-semibold text-gray-800">Total: R{order.total.toFixed(2)}</p>
