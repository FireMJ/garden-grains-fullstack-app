...
export default function SoupDetailPage() {
  const params = useParams();
  const router = useRouter();
  const { addToCart } = useCart();
  
  const slug = params.slug as string;
  const soupItem = soups.find(item => item.slug === slug);
    ...
    return (
      <div className="min-h-screen bg-[#1E4259] flex items-center justify-center">
        <div className="text-center text-white">
          <h1 className="text-4xl font-bold mb-4">Soup Not Found</h1>
          <button 
            onClick={() => router.push("/menu/soups")}
  ...
  const allAddOns = [...(soupItem.addOns || []), ...soupAddOns];
          ...
          <div className="relative h-80 md:h-96 rounded-2xl overflow-hidden">
            {!imageError ? (
              ...
              <div className="absolute inset-0 bg-gradient-to-br from-[#8B4513] to-[#D2691E] flex items-center justify-center">
                <span className="text-white/80 text-lg">Soup Image</span>
              </div>
            )}
          </div>
          ...
          <div className="space-y-6">
            <div>
              ...
              <p className="text-gray-300 mb-4">
                {soupItem.description}
              </p>
              
              <div className="text-2xl font-bold text-green-300">
                R{soupItem.price.toFixed(2)}
              </div>
            </div>
