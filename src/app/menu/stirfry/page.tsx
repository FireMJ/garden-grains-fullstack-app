      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-gray-300 max-w-2xl mx-auto">
            Fresh, flavorful stir-fries made with premium ingredients and authentic Asian-inspired sauces
          </p>
        </div>
            ...
            return (
              <div key={item.id} onClick={() => handleNavigate(item.slug)} className="bg-white/10 rounded-2xl shadow-lg cursor-pointer overflow-hidden transition transform hover:scale-105 hover:shadow-xl backdrop-blur-sm">
                ...
                <div className="p-6 flex flex-col gap-2">
                  ...
                  <span className="font-bold text-green-300 text-lg">R{item.price.toFixed(2)}</span>

                  {/* Base Options Preview */}
                  {item.baseOptions && (
                    <div className="text-xs text-gray-400">
                      Bases: {item.baseOptions.slice(0, 2).map(base => base.name).join(", ")}
                      {item.baseOptions.length > 2 && "..."}
                    </div>
                  )}
        ...
        <div className="mt-12 text-center">
          <div className="bg-white/10 rounded-2xl p-6 backdrop-blur-sm max-w-2xl mx-auto">
            ...
            <div className="flex flex-wrap justify-center gap-3">
              ...
              <Link href="/menu/juices" className="bg-[#2A5568] text-white px-4 py-2 rounded-lg hover:bg-[#1E4259] transition text-sm">Juices</Link>
