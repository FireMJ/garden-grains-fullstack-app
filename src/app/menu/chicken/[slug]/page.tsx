...
export default function ChickenDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  const router = useRouter();
  const { addToCart } = useCart();
  const [chickenItem, setChickenItem] = useState<ChickenItem | null>(null);
  const [selectedBasting, setSelectedBasting] = useState<BastingOption | null>(null);
  ...
  useEffect(() => {
    if (!slug) return;
    const item = chickenItems?.find((c: any) => c.slug === slug);
    if (item) {
      setChickenItem(item);
      if (item.bastingOptions && item.bastingOptions.length > 0) {
        setSelectedBasting(item.bastingOptions[0]);
      }
    }
  }, [slug]);
  ...
  const calculateTotal = () => {
    let total = chickenItem?.price || 0;
    
    if (selectedBasting && selectedBasting.price > 0) {
      total += selectedBasting.price;
    }
  ...
  const handleAddToCart = () => {
    if (!chickenItem) return;
    
    let itemName = chickenItem.name;
    if (selectedBasting && selectedBasting.name !== "No Basting") {
      itemName += ` (${selectedBasting.name} basting)`;
    }
    if (selectedFries) {
    ...
    const cartItem = {
      id: `${chickenItem.id}-${Date.now()}`,
      name: itemName,
      price: chickenItem.price,
      quantity: quantity,
      image: chickenItem.image,
      category: "chicken",
      description: chickenItem.description,
      basting: selectedBasting?.name,
    ...
    console.log("Adding chicken to cart:", cartItem);
    addToCart(cartItem);
    router.push("/cart");
  };

  if (!mounted || !chickenItem) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-green-600 mx-auto mb-4"></div>
          <p className="text-gray-600">Loading chicken details...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="bg-white shadow-sm sticky top-0 z-10">
        <div className="container mx-auto px-4 py-4">
          <Link href="/menu/chicken" className="inline-flex items-center text-gray-600 hover:text-green-600 transition">
      ...
      <div className="container mx-auto px-4 py-8 max-w-6xl">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900 mb-2">{chickenItem.name}</h1>
            <p className="text-gray-600 mb-4">{chickenItem.description}</p>
            <div className="text-2xl font-bold text-green-600 mb-6">R{chickenItem.price}</div>

            {chickenItem.tags && chickenItem.tags.length > 0 && (
              <div className="mb-6">
                <div className="flex flex-wrap gap-2">
                  {chickenItem.tags.map((tag, idx) => (
            ...
            {chickenBastingOptions && chickenBastingOptions.length > 0 && (
              <div className="mb-6">
                <h3 className="font-semibold text-gray-900 mb-2">Choose Your Basting</h3>
                <div className="flex flex-wrap gap-2">
                  {chickenBastingOptions.map((basting) => (
                    <button
                      key={basting.id}
                      onClick={() => setSelectedBasting(basting)}
                      className={`px-3 py-2 rounded-lg border text-sm transition ${
                        selectedBasting?.id === basting.id
                          ? 'border-green-500 bg-green-50 text-green-700'
                          : 'border-gray-300 hover:border-green-300'
                      }`}
                    >
                      {basting.name} {basting.price > 0 && `(+R${basting.price})`}
                    </button>
                  ))}
                </div>
              </div>
