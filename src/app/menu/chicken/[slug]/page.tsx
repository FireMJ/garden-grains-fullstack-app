...
export default function ChickenDetailPage({ params }: PageProps) {
  const router = useRouter();
  const { addToCart } = useCart();
  const [chickenItem, setChickenItem] = useState<ChickenItem | null>(null);
  const [selectedBasting, setSelectedBasting] = useState<any>(null);
  ...
  const handleAddToCart = () => {
    const cartItem = {
      ...
      name: chickenItem?.name || "",
      price: chickenItem?.price || 0,
      quantity: quantity,
          ...
          <div>
            ...
            <div className="mb-6">
              ...
              <div className="grid grid-cols-2 gap-2">
                {chickenItem.bastingOptions?.map((basting) => (
                  <button
                    key={basting.id}
                  ...
                  >
                    <span className="font-medium text-gray-900">{basting.name}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="mb-6">
              ...
              >
                {showUpsells ? '▼' : '▶'} Add Drink to Complete Your Meal
              </button>
